...
count_true_positives <- function(df, truth, estimate) {
    result <- df %>%
        group_by(.data[[truth]]) %>%
...
count_false_positives <- function(df, truth, estimate) {
    result <- df %>%
...
count_false_negatives <- function(df, truth, estimate) {
    result <- df %>%
        group_by(.data[[truth]]) %>%
        summarise(false_neg=sum(.data[[truth]]!=.data[[estimate]])) %>%
        rename(behaviour=.data[[truth]]) %>%
        replace(is.na(.), 0)
    return(result)
}
...
calc_fmeas <- function(precision, recall) {
    f_meas <- if_else(precision==0 | recall==0, 0,
...
calc_class_metrics <- function(df, truth, estimate) {
    true_pos <- count_true_positives(df, truth, estimate)
    false_pos <- count_false_positives(df, truth, estimate)
    false_neg <- count_false_negatives(df, truth, estimate)
    class_metrics <- full_join(true_pos, false_pos, by='behaviour') %>%
        full_join(., false_neg, by='behaviour') %>%
        replace(is.na(.), 0) %>%
...
calc_macro_metrics <- function(class_metrics,target_behaviours=NA,
                               group_folds=TRUE, n_predictions="grouped") {
    ...
    # if n_predictions = grouped, get count for each fold to calculate n_predictions
    # if n_predictions = ungrouped, n for class metrics is same in all rows (can use any without grouping)
    ...
    if(group_folds==TRUE) {
        macro_metrics <- class_metrics %>%
            group_by(fold) %>%
            summarise(n_predictions=first(n),
                      true_pos=sum(true_pos, na.rm=T),
                      precision=mean(precision, na.rm=T),
                      recall=mean(recall, na.rm=T),
                      f_meas=mean(f_meas, na.rm=T),
                      accuracy=true_pos/n_predictions)
    }
    if(group_folds==FALSE) {
        if(n_predictions=="grouped") {
            fold_summary <- class_metrics %>% group_by(fold) %>% summarise(n=n())
            n_predictions <- sum(fold_summary$n)
        }
        if(n_predictions=="ungrouped") {
            n_predictions <- first(class_metrics$n)
        }
        macro_metrics <- class_metrics %>%
...
map_class_metrics <- function(predictions, by_var, truth, pred) {
    ...
    fold_class_metrics <- map(split(predictions, by=by_var),
                             ~calc_class_metrics(.x, truth, pred)) %>%
        bind_rows(.id = by_var)
...
map_macro_metrics <- function(class_metrics, 
    ...
    if(multimodel==FALSE & cv==FALSE){
        macro_allbeh <- calc_macro_metrics(class_metrics, 
                                           behaviour_list_all,
                                           group_folds = FALSE,
                                           n_predictions = "ungrouped")
        macro_matingbeh <- calc_macro_metrics(class_metrics, 
                                              behaviour_list_mating,
                                              group_folds = FALSE,
                                              n_predictions = "ungrouped") %>%
            rename_with(~paste0(.x, "_mat"), true_pos:accuracy)
    
        macro_matingbeh$accuracy_mat <- NULL
    ...
    # requires model_fold column (with fold number in each row after _Fold)
        macro_allbeh <- map(split(class_metrics, by="model_fold"),
                        ~calc_macro_metrics(.x, behaviour_list_all)) %>%
            bind_rows(.id = "model_fold") %>%
            mutate(model=as.factor(sub("_Fold.*", "", model_fold))) %>%
            average_macro_metrics(., multimodel=TRUE)
    
        macro_matingbeh <- map(split(class_metrics, by="model_fold"),
                     ~calc_macro_metrics(.x, behaviour_list_mating)) %>%
            bind_rows(.id = "model_fold") %>%
            mutate(model=as.factor(sub("_Fold.*", "", model_fold))) %>%
            average_macro_metrics(., multimodel=TRUE) %>%
            rename_with(~paste0(.x, "_mat"), precision:accuracy_sd)
    
        macro_matingbeh$accuracy_mat <- NULL
        macro_matingbeh$accuracy_sd_mat <- NULL
    
        macro_metrics <- full_join(macro_allbeh, macro_matingbeh, by='model')
        return(macro_metrics)
    }
    
    if(multimodel==TRUE & cv==FALSE){
        macro_allbeh <- map(split(class_metrics, by="model"),
                            ~calc_macro_metrics(.x, behaviour_list_all,
                                                group_folds=FALSE,
                                                n_predictions="ungrouped")) %>%
            bind_rows(.id = "model")
        
        macro_matingbeh <- map(split(class_metrics, by="model"),
                            ~calc_macro_metrics(.x, behaviour_list_mating,
                                                group_folds=FALSE,
                                                n_predictions="ungrouped")) %>%
            bind_rows(.id = "model") %>%
            rename_with(~paste0(.x, "_mat"), precision:accuracy)
    ...
    if(multimodel==FALSE & cv==TRUE){
        macro_allbeh <- map(split(class_metrics, by="fold"),
                            ~calc_macro_metrics(.x, behaviour_list_all)) %>%
            bind_rows(.id = "fold")
        
        macro_matingbeh <- map(split(class_metrics, by="fold"),
                            ~calc_macro_metrics(.x, behaviour_list_mating)) %>%
            bind_rows(.id = "fold") %>%
...
import_hmm_preds <- function(directory, filename) {
    ...
    df <- fread(here(directory, filename)) %>% 
        mutate(fold = substr(filename,1,nchar(filename)-4))
    return(df)
}
...
hmm_predictions <- list.files(path_hmm_models, pattern=".csv") %>%
    map_df(.,
           ~import_hmm_preds(directory=path_hmm_models,
           filename=.x))

hmm_predictions <- hmm_predictions %>%
    rename(model_fold=fold) %>% 
    mutate(model_fold=str_replace(model_fold, "fold", "Fold"),
           validation_type=case_when(model_fold=="test" ~ "test",
                                     TRUE ~ "CV"),
           fold=sub(".*_", "", model_fold))
...
hmm_fold_class_metrics <- map_class_metrics(hmm_cv_predictions,
                                                 ...
                                                 truth='truth_beh',
                                                 pred='pred_beh') %>%
    mutate(fold=sub(".*Fold", "", model_fold),
           model=sub("_Fold.*", "", model_fold))
...
hmm_test_macro <- map_macro_metrics(hmm_test_class,
    ...
    mutate(validation_type="test",
           model="timestrat",
           split_type="timestrat")
                                     
hmm_macro_top <- cbind(model_type="HMM",
                       full_join(hmm_cv_macro, hmm_test_macro))

hmm_macro_top %>% flextable()
```

```{r Summarise hmm class metrics}
hmm_cv_class <- map(split(hmm_fold_class_metrics, by="model"),
...
rf_predictions$model <- paste0(rf_predictions$model_category,
                               "_mtry", rf_predictions$mtry,
                               "_minn", rf_predictions$min_n)

rf_predictions$model_fold <- factor(paste0(rf_predictions$model,
...
rf_fold_class_metrics$model <- sub("_Fold.*", "", rf_fold_class_metrics$model_fold)
...
rf_cv_macro <- rf_cv_macro %>%
    mutate(split_type=case_when(grepl("LSIO", model) ~ "LSIO",
                                grepl("strat", model) ~ "randstrat",
                                grepl("timesplit", model) ~ "timestrat"),
           transitions_in_test=case_when(grepl("ToutAll", model) ~ "no",
                                         grepl("Tout", model) ~ "yes",
                                         grepl("Tin", model) ~ "yes"))

rf_cv_macro$model_folder <- sub("\\_mtry.*", "", rf_cv_macro$model)
rf_cv_macro$min_n <- sub(".*minn", "", rf_cv_macro$model)
rf_cv_macro$mtry <- str_match(rf_cv_macro$model, "mtry\\s*(.*?)\\s*_")[,2]
```
...
rf_cv_macro %>%
    filter(split_type=="LSIO" & transitions_in_test=="yes") %>%
    arrange(desc(f_meas_mat)) %>%
    select(model, f_meas, precision, recall, 
...
best_tstrat_t <- rf_cv_macro %>%
    filter(split_type=="timestrat" & transitions_in_test=="yes") %>%
    arrange(desc(f_meas_mat)) %>%
    head(1)
...
best_tstrat_nt <- rf_cv_macro %>%
    filter(split_type=="timestrat" & transitions_in_test=="no") %>%
    arrange(desc(f_meas_mat)) %>%
    head(1)
...
rf_cv_macro %>%
    filter(split_type=="randstrat" & transitions_in_test=="yes") %>%
    arrange(desc(f_meas_mat)) %>%
    select(model, f_meas, precision, recall, recall_sd, 
           f_meas_mat, precision_mat, recall_mat, f_meas_sd_mat) %>%
    flextable() %>%
    colformat_double(digits=3)
```
...
rf_cv_macro_top <- 
    rf_cv_macro %>%
    filter(model==best_LSIO_nt$model | model==best_LSIO_t$model |
               model==best_rstrat_nt$model | model==best_rstrat_t$model |
               model==best_tstrat_nt$model | model==best_tstrat_t$model) %>%
    mutate(validation_type="CV")
```
...
timestrat_folds <- fread(path_timesplit_folds, data.table=FALSE)
...
timestrat_folds <- timestrat_folds %>%
...
tstrat_t_test_list <- timestrat_folds %>% filter(fold=="test")
tstrat_t_test_dat <- dat %>% filter(recording_window %in% tstrat_t_test_list$recording_window)
tstrat_t_train_dat <- dat %>% filter(!(recording_window %in% tstrat_t_test_list$recording_window))

rstrat_t_test_list <- rstrat_folds %>% filter(fold=="test")
rstrat_t_test_dat <- dat %>% filter(recording_window %in% rstrat_t_test_list$recording_window)
rstrat_t_train_dat <- dat %>% filter(!(recording_window %in% rstrat_t_test_list$recording_window))
...
best_tstrat_t_testpreds <- get_test_predictions(best_tstrat_t_fit, test_data = tstrat_t_test_dat)

best_tstrat_t_testpreds <- best_tstrat_t_testpreds %>%
    mutate(model=best_tstrat_t$model)
```
...
best_tstrat_nt_testpreds <- get_test_predictions(best_tstrat_nt_fit, test_data = tstrat_nt_test_dat)

best_tstrat_nt_testpreds <- best_tstrat_nt_testpreds %>%
    mutate(model=best_tstrat_nt$model)
```
...
best_rstrat_t_testpreds <- get_test_predictions(best_rstrat_t_fit, test_data = rstrat_t_test_dat)

best_rstrat_t_testpreds <- best_rstrat_t_testpreds %>%
    mutate(model=best_rstrat_t$model)
```
...
best_rstrat_nt_testpreds <- get_test_predictions(best_rstrat_nt_fit, test_data = rstrat_nt_test_dat)

best_rstrat_nt_testpreds <- best_rstrat_nt_testpreds %>%
    mutate(model=best_rstrat_nt$model)
```
...
if(behaviours=="reduced") {
    ...
    rf_test_predictions$truth_beh <- reduce_beh(as.character(rf_test_predictions$original_truth))
    rf_test_predictions$pred_beh <- reduce_beh(as.character(rf_test_predictions$original_pred))
}
```
...
rf_test_class <- 
    map_class_metrics(rf_test_predictions, "model", "truth_beh", "pred_beh")
...
rf_macro_top %>%
    select(model, split_type, validation_type, transitions_in_test, 
...
rf_cv_class_top <- map(split(rf_fold_class_top, by="model"),
                        ~average_class_metrics(.x)) %>%
    bind_rows(.id = "model") %>%
    mutate(split_type=case_when(grepl("LSIO", model) ~ "LSIO",
                                grepl("strat", model) ~ "randstrat",
...
nn_predictions <- full_join(nn_predictions_tin, nn_predictions_tout) %>%
    mutate(fold=str_replace(fold,"fold","Fold"),
           model=paste0("NN","_",transitions),
           model_fold=paste0(model,"_",fold))
...
nn_fold_class_metrics <- map_class_metrics(nn_predictions,
    ...
    mutate(fold=sub(".*Fold", "", model_fold, ignore.case=TRUE),
           model=sub("_Fold.*", "", model_fold, ignore.case=TRUE),
           split_type=model)
...
nn_predictions_top <- nn_predictions %>%
    filter(model==nn_macro_top$model)
...
nn_class_top <- nn_fold_class_metrics %>%
    ...
    mutate(model=nn_macro_top$model,
           split_type="LSIO")
