...
beh_ind <- windowed_dat %>%
    group_by(majority_behaviour, ruff_id) %>%
    rename(Behaviour = majority_behaviour) %>%
    summarise(`total duration (s)`=n(),
              `n bouts`=n_distinct(beh_event_id))
...
```{r Format individual counts}
beh_ind_wide <- beh_ind %>%
    select(!`n bouts`) %>%
    pivot_wider(.,
                names_from = ruff_id,
                values_from = `total duration (s)`)
...
beh_ind_wide
```
```{r Create flextable for individual behaviours}
beh_col_width <- 3.5

beh_ind_flex <- beh_ind_wide %>%
    flextable() %>%
    align(align = "left", part="all") %>%
    # colformat_double(j=4,
    #                  digits=2) %>%
    ...
    bold(part="header") %>%
    # ---- Set width ----
    width(width=(land_width-beh_col_width)/(ncol(beh_ind_wide)-1), unit="cm") %>%
    width(j=1, beh_col_width, "cm")

beh_ind_flex
```

```{r Format individual events}
event_ind_wide <- beh_ind %>%
    select(!`total duration (s)`) %>%
    pivot_wider(.,
                names_from = ruff_id,
                values_from = `n bouts`)

event_ind_wide$Behaviour <- rename_behaviours(event_ind_wide$Behaviour)
event_ind_wide$Behaviour <- order_behaviours(event_ind_wide$Behaviour)
event_ind_wide <- event_ind_wide %>% 
    arrange(Behaviour) %>%
     mutate_all(., ~replace_na(.,0))
event_ind_wide
```
...
event_ind_flex <- event_ind_wide %>%
    ...
    bold(part="header") %>%
    # ---- Set width ----
    width(width=(land_width-beh_col_width)/(ncol(event_ind_wide)-1), unit="cm") %>%
    width(j=1, beh_col_width, "cm")
...
rec_info_flex <- 
   ...
   # bold(i = ~ !is.na(behaviour_type), part="body") %>%
    ...
    bold(part="header") %>%
    # ---- Set width ----
    width(width=(port_width-first_col_width-second_col_width)/(ncol(rec_info_wide)-2), unit="cm") %>%
